...
i <-9
while(i<34){
  Red_19[,a] <- round(Red_19[,i]/Red_19[,8],3) #count of each category/contribuenti
  a<-a+1
  i<-i+2
}

Red_19 <- Red_19[,-c(51,52)] #remove incidence of regional and municipal tax

colnames(Red_19)[8:34]<-c("taxpayers","count_lavdip","lavdip","count_ret","ret","count_piva","piva","count_regio","regio","count_comu","comu","count_0less","0less","count_10k","10k","count_15k","15k",
                          "count_26k","26k","count_55k","55k","count_75k","75k",
                          "count_120k","120k","count_120+k","120+k")
#join PAs Bolzano & Trento

Red_19<- Red_19%>%
  mutate(Regione = ifelse((`Codice Istat Regione` == 4), "Trentino Alto Adige",Regione))

###Group by region and group by regXdegurba###
#####regionXdegurba#####
units_rd_19<- classified_units%>%
...
library(data.table)
weighted_inc_19 <-setDT(units_rd_19)[,  Map(weighted.mean, mget(grep('%', names(units_rd_19), value=TRUE)), 
                                            mget(grep('count_', names(units_rd_19), value=TRUE))), by = by]  
weighted_inc_19 <- weighted_inc_19[,-c(15,16)]
#####compute aggregate values - region x degurba level######
regdeg_19 <- Red_19_gbregdeg%>%
  left_join(weighted_inc_19[,-3],join_by("Regione","DGURBA"))
regdeg_19[is.na(regdeg_19)] <- 0

empty_cols <- c('avglavdip','avgret','avgpiva','avgadreg','avgadcom', 'avg0less', 
                'avg10k','avg15k','avg26k','avg55k','avg75k','avg120k','avg120+k')

regdeg_19[,empty_cols]<-NA
a <- 42
i <- 6
while(i<31){
  regdeg_19[,a] <- regdeg_19[,i]/regdeg_19[,i-1]
  a<-a+1
  i<-i+2
}
###########ESTIMATING POVERTY#############
...
modelling_estimates19 <- regdeg_19%>%
  left_join(harmonize_region_code,join_by("Regione" == "names"))

modelling_estimates19 <- modelling_estimates19%>%
